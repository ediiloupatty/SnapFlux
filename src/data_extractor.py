...
def get_tabung_terjual_direct(driver):
    ...
    try:
        time.sleep(1.5)
        print(
            "🚀 Mengambil data tabung terjual langsung menggunakan lokasi yang sudah diketahui..."
        )
                    ...
                    try:
                        ...
                        for element in elements:
                            ...
                            if (
                                ...
                                print(
                                    f"✅ Data tabung terjual ditemukan dengan XPath fallback!"
                                )
                                print(f"📝 Text: '{text}'")
...
def get_customer_list_direct(driver):
    ...
    try:
        time.sleep(1.0)
        print(
            "🚀 Mengambil data list pembeli langsung menggunakan lokasi yang sudah diketahui..."
        )
        ...
        try:
            ...
            customer_selectors = [
                ...
                (
                    By.XPATH,
                    "//div[@class='styles_listTransactionRoot__pvz4r mantine-1uguyhf']",
                ),
            ]
            ...
            if elements:
                ...
                for idx, element in enumerate(elements):
                    try:
                        text = element.text.strip()
                        if text and (
                            "tabung" in text.lower() and "lpg" in text.lower()
                        ):
                ...
                if customer_data:
                    ...
                    for idx, customer in enumerate(customer_data[:3]):
                        ...
                        print(
                            f"   🎨 Class: '{customer['element_info']['class'] or 'N/A'}'"
                        )
                ...
                print(
                    "❌ Container pembeli tidak ditemukan dengan class yang diketahui"
                )
                return None
...
def read_nik_from_excel():
    ...
    try:
        ...
        if ws is None:
            print("❌ Workbook tidak memiliki sheet aktif")
            return None
    ...
    try:
        ...
        try:
            ...
            if buttons:
                ...
                print(
                    f"✅ Tombol LANJUTKAN PENJUALAN ditemukan: '{continue_button.text}'"
                )
            else:
                ...
                if buttons:
                    ...
                    for button in buttons:
                        if "LANJUTKAN" in button.text.strip().upper():
                            ...
                            print(
                                f"✅ Tombol LANJUTKAN ditemukan dengan class: '{button.text}'"
                            )
                            break
        except Exception as e:
        ...
        time.sleep(
            0.3
        )  # Tunggu halaman load (reduced untuk total delay maksimal 1 detik)
        ...
        try:
            ...
            if cek_pesanan_success:
                print(
                    f"✅ CEK PESANAN berhasil diklik langsung - lanjutkan proses (waktu: {cek_elapsed:.2f} detik)"
                )
                return True
            else:
                print(
                    f"⚠️ CEK PESANAN tidak ditemukan atau gagal - langsung cek TUTUP (waktu: {cek_elapsed:.2f} detik)"
                )
                # Langsung skip ke TUTUP tanpa delay
        except Exception as e:
            cek_elapsed = time_module.time() - cek_start_time
            print(
                f"⚠️ Error saat mencoba klik CEK PESANAN: {str(e)} - langsung cek TUTUP (waktu: {cek_elapsed:.2f} detik)"
            )
            # Langsung skip ke TUTUP tanpa delay
        ...
        try:
            ...
            fast_selectors = [
                ...
                (
                    By.XPATH,
                    "//button[translate(normalize-space(text()), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')='TUTUP']",
                ),  # Exact match normalized (prioritas 1)
                (
                    By.XPATH,
                    "//button[normalize-space(text())='TUTUP' or normalize-space(text())='Tutup' or normalize-space(text())='tutup']",
                ),  # Variasi case (prioritas 2)
                # Contains dengan normalize-space
                (
                    By.XPATH,
                    "//button[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'TUTUP')]",
                ),  # Contains normalized (prioritas 3)
                # Link dengan variasi yang sama
                (
                    By.XPATH,
                    "//a[translate(normalize-space(text()), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')='TUTUP']",
                ),  # Link exact match (prioritas 4)
                (
                    By.XPATH,
                    "//a[normalize-space(text())='TUTUP' or normalize-space(text())='Tutup' or normalize-space(text())='tutup']",
                ),  # Link variasi case (prioritas 5)
                (
                    By.XPATH,
                    "//a[contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'TUTUP')]",
                ),  # Link contains (prioritas 6)
                # Role button
                (
                    By.XPATH,
                    "//*[@role='button' and contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'TUTUP')]",
                ),  # Role button (prioritas 7)
                # Fallback: cari semua button/link yang mengandung TUTUP (case-insensitive, normalize-space)
                (
                    By.XPATH,
                    "//*[self::button or self::a or @role='button'][contains(translate(normalize-space(.), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'TUTUP')]",
                ),  # Semua button/link (prioritas 8)
            ]

            print(f"🔍 Mencari tombol TUTUP dengan {len(fast_selectors)} selector...")

            # Coba selector cepat terlebih dahulu
            selector_attempt_start = time_module.time()  # Initialize before loop
            for idx, (selector_type, selector_value) in enumerate(fast_selectors, 1):
                ...
                try:
                    ...
                    elements = driver.find_elements(selector_type, selector_value)
                    selector_attempt_elapsed = (
                        time_module.time() - selector_attempt_start
                    )
                    elements_count = len(elements)
                    print(
                        f"  🔍 Selector #{idx}: menemukan {elements_count} elemen (waktu: {selector_attempt_elapsed:.3f}s)"
                    )
                        ...
                        try:
                            ...
                            if btn.is_displayed() and btn.is_enabled():
                                ...
                                if "TUTUP" in normalized_text:
                                    ...
                                    starts_with_tutup = normalized_text.startswith(
                                        "TUTUP"
                                    )
                                    ...
                                    if (
                                        ...
                                        print(
                                            f"     Text langsung: '{button_direct_text[:100]}...' (length: {len(button_direct_text)})"
                                        )
                                        print(
                                            f"     Text normalized: '{normalized_text}'"
                                        )
                                        print(
                                            f"     Match reason: exact={is_exact_match}, short={is_short_text}, starts={starts_with_tutup}, ends={ends_with_tutup}"
                                        )
                                        print(
                                            f"✅ Tombol TUTUP ditemukan dengan selector #{idx} (waktu: {elapsed:.2f} detik)"
                                        )
                                        break
                                    else:
                                        print(
                                            f"  ⚠️ Element #{btn_idx} mengandung TUTUP tapi tidak memenuhi kriteria: '{normalized_text[:50]}...' (length: {text_length}, skip)"
                                        )
                                else:
                                    print(
                                        f"  ⚠️ Element #{btn_idx} tidak mengandung TUTUP: '{normalized_text[:50]}...'"
                                    )
                            else:
                                print(
                                    f"  ⚠️ Element #{btn_idx} tidak displayed/enabled: '{normalized_text[:50]}...'"
                                )
                        except Exception as btn_e:
                            print(
                                f"  ⚠️ Error saat cek element #{btn_idx}: {str(btn_e)[:50]}"
                            )
                            continue
                    ...
                    print(
                        f"  ❌ Selector #{idx} error (waktu: {selector_attempt_elapsed:.3f}s): {str(sel_e)[:80]}"
                    )
                    continue
            ...
            if not target_tutup:
                ...
                if elapsed < max_tutup_time:
                    try:
                        print(
                            f"  🔄 Mencoba mencari TUTUP di modal/dialog (fallback 2)..."
                        )
                            ...
                            try:
                                ...
                                for modal_btn in reversed(
                                    ...
                                    if (
                                        ...
                                        if "TUTUP" in btn_direct_text and (
                                            btn_direct_text == "TUTUP"
                                            or len(btn_direct_text) < 15
                                            or btn_direct_text.startswith("TUTUP")
                                            or btn_direct_text.endswith("TUTUP")
                                        ):
                                            target_tutup = modal_btn
                                            elapsed = (
                                                time_module.time() - tutup_start_time
                                            )
                                            print(
                                                f"  ✅ Tombol TUTUP ditemukan di modal (waktu: {elapsed:.2f} detik)"
                                            )
                                            break
                                if target_tutup:
                                    break
                        ...
                        print(
                            f"  ⚠️ Error saat mencari TUTUP di modal: {str(modal_e)[:50]}"
                        )
            ...
            if not target_tutup:
                ...
                if elapsed < max_tutup_time:
                    try:
                        print(
                            f"  🔄 Mencoba JavaScript fallback untuk mencari TUTUP..."
                        )
                        ...
                        if tutup_info:
                            print(
                                f"  🪟 JavaScript menemukan {len(tutup_info)} kandidat tombol TUTUP"
                            )
                            for info in tutup_info:
                                ...
                                try:
                                    ...
                                    search_selectors = []
                                    if info["id"]:
                                        search_selectors.append(
                                            f"//{info['tagName']}[@id='{info['id']}']"
                                        )
                                    if info["classes"]:
                                        # Ambil class pertama
                                        first_class = (
                                            info["classes"].split()[0]
                                            if info["classes"]
                                            else None
                                        )
                                        if first_class:
                                            search_selectors.append(
                                                f"//{info['tagName']}[contains(@class, '{first_class}')]"
                                            )
                                    # Fallback: cari dengan text
                                    search_selectors.append(
                                        f"//{info['tagName']}[normalize-space(.)='{info['text'].strip()}']"
                                    )

                                    for search_sel in search_selectors:
                                        try:
                                            found_btn = driver.find_element(
                                                By.XPATH, search_sel
                                            )
                                            if (
                                                found_btn.is_displayed()
                                                and found_btn.is_enabled()
                                            ):
                                                ...
                                                print(
                                                    f"✅ Tombol TUTUP ditemukan dengan JavaScript fallback (waktu: {elapsed:.2f} detik): '{info['textUpper']}'"
                                                )
                                                break
                                        except:
                                            continue
            ...
            if target_tutup:
                ...
                for attempt in range(
                    2
                ):  # Kurangi dari 3 ke 2 attempt untuk lebih cepat
                    try:
                        if attempt == 0:
                        ...
                        clicked = True
                        print(
                            f"✅ Tombol TUTUP berhasil diklik (attempt {attempt + 1})"
                        )
                        break
                    except Exception as e:
                        if attempt == 0:
                ...
                if clicked:
                    ...
                    try:
                        ...
                        if catat_reopen:
                            print(
                                "✅ Berhasil klik ulang Catat Penjualan setelah TUTUP"
                            )
                        else:
                            print(
                                "⚠️ Gagal klik ulang Catat Penjualan, tapi akan lanjut"
                            )
                    except Exception as e_reopen:
                        print(
                            f"⚠️ Error saat klik ulang Catat Penjualan: {str(e_reopen)}"
                        )

                    print(
                        "✅ Popup TUTUP diklik dan Catat Penjualan sudah dibuka lagi - siap untuk NIK berikutnya"
                    )
        ...
        try:
            ...
            nib_selectors = [
                (
                    By.XPATH,
                    "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'LENGKAPI NIB')]",
                ),
                (
                    By.XPATH,
                    "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'NOMOR INDUK BERUSAH')]",
                ),
                (By.XPATH, "//*[contains(@class, 'icon-warning-nib')]"),
                (
                    By.XPATH,
                    "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'SEGERA LENGKAPI NIB')]",
                ),
            ]
            for how, sel in nib_selectors:
                if driver.find_elements(how, sel):
                    nib_detected = True
                    print(
                        "📋 Popup NIB terdeteksi — mencari tombol 'Nanti Saja, Lanjut Transaksi'..."
                    )
                    break

            if nib_detected:
                # Cari tombol "Nanti Saja, Lanjut Transaksi" atau "Lanjut Transaksi"
                lanjut_transaksi_buttons = driver.find_elements(
                    By.XPATH,
                    "//*[self::button or self::a][contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'LANJUT TRANSAKSI') or contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'NANTI SAJA')]",
                )
                ...
                for btn in lanjut_transaksi_buttons:
                    ...
                    try:
                        if ("LANJUT" in label and "TRANSAKSI" in label) or (
                            "NANTI" in label and "SAJA" in label
                        ):
                            if btn.is_displayed() and btn.is_enabled():
                                target_lanjut = btn
                                print(
                                    f"✅ Tombol 'Nanti Saja, Lanjut Transaksi' ditemukan: '{btn.text.strip()}'"
                                )
                                break
                    except:
                        continue
                ...
                if target_lanjut:
                    ...
                    try:
                        target_lanjut.click()
                        print(
                            "✅ Tombol 'Nanti Saja, Lanjut Transaksi' berhasil diklik (normal click)"
                        )
                    except Exception as e:
                        try:
                            driver.execute_script(
                                "arguments[0].click();", target_lanjut
                            )
                            print(
                                "✅ Tombol 'Nanti Saja, Lanjut Transaksi' berhasil diklik (JavaScript click)"
                            )
                        except Exception as e2:
                            print(
                                f"❌ Gagal mengklik tombol 'Nanti Saja, Lanjut Transaksi': {str(e2)}"
                            )
                    ...
                    try:
                        ...
                        if cek_pesanan_success:
                            print(
                                "✅ CEK PESANAN berhasil diklik setelah handle NIB - lanjutkan proses"
                            )
                            return True
                        else:
                            print(
                                "⚠️ CEK PESANAN tidak ditemukan setelah handle NIB - lanjut cek popup lain"
                            )
                    except Exception as e:
                        print(
                            f"⚠️ Error saat mencoba klik CEK PESANAN setelah handle NIB: {str(e)} - lanjut cek popup lain"
                        )
        except Exception as e:
        ...
        try:
            ...
            if cek_pesanan_success:
                print(
                    "✅ CEK PESANAN berhasil diklik setelah handle popup - lanjutkan proses"
                )
                return True
            else:
            ...
            print(
                f"⚠️ Error saat mencoba klik CEK PESANAN setelah handle popup: {str(e)}"
            )
            return False
...
def click_cek_pesanan(driver):
    ...
    try:
        ...
        candidate_selectors = [
            (
                By.XPATH,
                "//button[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'CEK PESANAN')]",
                True,
            ),  # Text-based dengan wait dinamis (prioritas)
            (
                By.XPATH,
                "//html[1]/body[1]/div[1]/div[1]/div[1]/main[1]/div[1]/div[1]/div[1]/div[1]/div[1]/div[2]/form[1]/div[4]/div[1]/button[1]",
                False,
            ),  # XPath langsung, no wait (fallback cepat)
        ]

        target = None
        max_total_time = (
            0.8  # Maksimal total waktu pencarian: 0.8 detik (dipercepat lagi)
        )
        selector_index = 0
        for how, value, use_wait in candidate_selectors:
            ...
            if elapsed_time >= max_total_time:
                print(
                    f"⏱️ Waktu pencarian melewati batas {max_total_time} detik - stop mencari dan lanjut ke flow popup"
                )
                elapsed_time = time_module.time() - start_time
                print(
                    f"❌ Tombol CEK PESANAN tidak ditemukan (waktu: {elapsed_time:.2f} detik) - lanjut ke flow popup"
                )
            ...
            try:
                ...
                print(
                    f"🔍 Selector #{selector_index} ({selector_type}): Mencoba selector {how}..."
                )
                ...
                if use_wait:
                    try:
                        ...
                        if remaining_time <= 0.05:
                            ...
                            print(
                                f"  ⚠️ Skip selector #{selector_index}: tidak ada waktu tersisa (remaining: {remaining_time:.3f}s)"
                            )
                            continue

                        # Gunakan wait yang sesuai dengan sisa waktu (maksimal 0.3 detik untuk cepat)
                        wait_timeout = min(
                            0.3, max(0.05, remaining_time - 0.05)
                        )  # Kurangi 0.05 untuk margin
                        print(
                            f"  ⏱️ WebDriverWait timeout: {wait_timeout:.3f} detik (remaining time: {remaining_time:.3f}s)"
                        )
                        ...
                        try:
                            ...
                            wait_remaining.until(
                                EC.visibility_of_element_located((how, value))
                            )
                            wait_result = "SUCCESS"
                            wait_elapsed = time_module.time() - selector_start_time
                            print(
                                f"  ✅ WebDriverWait berhasil menemukan elemen (waktu: {wait_elapsed:.3f}s)"
                            )
                        except Exception as wait_e:
                            wait_result = f"TIMEOUT: {str(wait_e)[:50]}"
                            wait_elapsed = time_module.time() - selector_start_time
                            print(
                                f"  ❌ WebDriverWait gagal (waktu: {wait_elapsed:.3f}s) - {wait_result}"
                            )
                            continue
                        ...
                        print(
                            f"  🔍 find_elements: menemukan {elements_count} elemen (waktu: {find_elapsed:.3f}s)"
                        )
                            ...
                            try:
                                ...
                                if is_valid:
                                    ...
                                    if is_displayed:
                                        ...
                                        target = el
                                        selector_elapsed = (
                                            time_module.time() - selector_start_time
                                        )
                                        if label:
                                            print(
                                                f"  ✅ Element #{idx + 1} cocok! Label: '{label}'"
                                            )
                                        else:
                                            print(
                                                f"  ✅ Element #{idx + 1} cocok! (label kosong, tapi selector spesifik dan elemen displayed)"
                                            )
                                        print(f"  📍 Posisi button: {position_info}")
                                        print(
                                            f"  ✅ Tombol CEK PESANAN ditemukan dengan selector #{selector_index} ({selector_type}) - waktu: {selector_elapsed:.3f}s"
                                        )
                                        break
                                    else:
                                        print(
                                            f"  ⚠️ Element #{idx + 1} tidak displayed setelah retry: '{label if label else 'N/A'}'"
                                        )
                                else:
                                    print(
                                        f"  ⚠️ Element #{idx + 1} tidak cocok: label='{label[:30] if label else 'kosong'}...'"
                                    )
                            except Exception as el_e:
                                print(
                                    f"  ⚠️ Error saat cek element #{idx + 1}: {str(el_e)[:50]}"
                                )
                                continue
                        if target:
                            break
                        ...
                        wait_elapsed = time_module.time() - selector_start_time
                        print(
                            f"  ❌ Selector #{selector_index} gagal (waktu: {wait_elapsed:.3f}s): {str(wait_err)[:80]}"
                        )
                        continue
                else:
                    ...
                    for idx, el in enumerate(elements):
                        try:
                            ...
                            if selector_index == 1:
                                # Selector pertama harus punya text yang valid
                                is_valid = (
                                    "CEK" in label_upper and "PESANAN" in label_upper
                                )
                            else:
                                # Selector kedua (XPath langsung) lebih fleksibel - terima jika text valid atau label kosong
                                is_valid = (
                ...
                print(
                    f"  ❌ Selector #{selector_index} error (waktu: {selector_elapsed:.3f}s): {str(e)[:80]}"
                )
                continue
        ...
        if not target:
            print(
                f"❌ Tombol CEK PESANAN tidak ditemukan setelah mencoba {selector_index} selector (waktu: {elapsed_time:.2f} detik) - lanjut ke flow popup"
            )
            return False
        ...
        print(
            f"🔍 Info tombol: Displayed={target.is_displayed()}, Enabled={target.is_enabled()}"
        )
        ...
        try:
            ...
            print(
                f"✅ Tombol CEK PESANAN berhasil diklik (normal click, waktu: {click_attempt_elapsed:.3f}s)"
            )
        except Exception as e1:
            normal_click_elapsed = time_module.time() - click_attempt_start
            print(
                f"  ❌ Normal click gagal (waktu: {normal_click_elapsed:.3f}s): {str(e1)[:80]}"
            )
            try:
                # Fallback: JavaScript click
                js_click_start = time_module.time()
                ...
                print(
                    f"✅ Tombol CEK PESANAN berhasil diklik (JavaScript click, waktu: {js_click_elapsed:.3f}s)"
                )
            except Exception as e2:
                js_click_elapsed = time_module.time() - js_click_start
                print(f"❌ Gagal klik tombol CEK PESANAN:")
                print(
                    f"   - Normal click: {str(e1)[:80]} (waktu: {normal_click_elapsed:.3f}s)"
                )
                print(
                    f"   - JavaScript click: {str(e2)[:80]} (waktu: {js_click_elapsed:.3f}s)"
                )
                return False
        ...
        elapsed_time = time_module.time() - start_time
        print(
            f"❌ Error klik CEK PESANAN: {str(e)} (total waktu: {elapsed_time:.2f} detik)"
        )
        return False
...
def click_proses_penjualan(driver):
    ...
    try:
        ...
        candidate_selectors = [
            (
                By.XPATH,
                "//html[1]/body[1]/div[1]/div[1]/div[1]/main[1]/div[1]/div[1]/div[1]/div[1]/div[1]/div[2]/div[2]/div[1]/div[4]/div[1]/button[1]",
            ),
            (
                By.XPATH,
                "//button[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'PROSES PENJUALAN')]",
            ),
            (
                By.XPATH,
                "//button[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'PROSES PESANAN')]",
            ),
            (
                By.XPATH,
                "//*[self::button or self::a][contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'PROSES')]",
            ),
        ...
        for how, value in candidate_selectors:
            try:
                ...
                for el in elements:
                    ...
                    if (
                        "PROSES" in label
                        and ("PENJUALAN" in label or "PESANAN" in label)
                        and el.is_enabled()
                        and el.is_displayed()
                    ):
                        target = el
...
def wait_for_captcha_and_success(driver, max_wait_seconds=180):
    ...
    try:
        ...
        while waited < max_wait_seconds:
            try:
                ...
                indicators = [
                    (
                        By.XPATH,
                        "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'LUNAS')]",
                    ),
                    (
                        By.XPATH,
                        "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'KIRIM STRUK KE PELANGGAN')]",
                    ),
                    (
                        By.XPATH,
                        "//*[contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'INFORMASI TRANSAKSI PELANGGAN')]",
                    ),
                ]
                ...
                if success_detected:
                    print(
                        "✅ Halaman sukses transaksi terdeteksi (LUNAS / Informasi Transaksi)."
                    )
                    return True
            except Exception:
                pass
        ...
        print(
            "❌ Timeout menunggu penyelesaian captcha / halaman sukses tidak terdeteksi."
        )
        return False
...
def click_kembali_ke_halaman_utama(driver):
    ...
    try:
        ...
        candidates = [
            (
                By.XPATH,
                "//*[self::button or self::a][contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'KEMBALI KE HALAMAN UTAMA')]",
            ),
            (
                By.XPATH,
                "//button[contains(@class,'button') and contains(translate(., 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'KEMBALI')]",
            ),
            (By.CLASS_NAME, "mantine-Button-root"),
        ]
